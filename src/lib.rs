...
impl Config {
    pub fn new(args: &[String]) -> Result<Config, &str> {
        if args.len() < 3 {
            return Err("You have not provided enough arguments");
        }
 ...
 * Recall that the lifetime parameters specify which argument lifetime is connected to the lifetime of the return value.
 * 
...
pub fn search_case_insensitive<'a>(query: &str, contents: &'a str) -> Vec<&'a str> {
    // query is now a String & not a string slice, coz calling to_lowercase() creates new data rather than referencing existing data
